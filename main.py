...
from src.model import get_bands_by_name, get_bands, get_band_by_id, get_persons_by_first_name, get_persons_by_last_name,Band
app = FastAPI()
...
def get_band(offset: int=0, limit:int=Query(default=10,lte=100)):
    return get_bands(offset,limit)


@app.get("/bands/{band_id}", response_model=Band)
def read_band_by_id(band_id: int, q: Union[str, None] = None):
